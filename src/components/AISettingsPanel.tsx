      ...
      <div className="w-full max-w-3xl rounded-2xl border border-slate-200 bg-white shadow-xl">
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-6 py-5">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold text-slate-950">选择模型服务商</h2>
            <p className="mt-2 text-sm leading-6 text-slate-600">配置后，知识点提取、生成题目和错因诊断会优先调用真实大模型；失败时自动回退 Mock。</p>
          </div>
          <button onClick={onClose} className="focus-ring rounded-xl p-2 text-slate-500 hover:bg-slate-100 hover:text-slate-900" aria-label="关闭 AI 设置">
            <X className="h-5 w-5" />
          </button>
        </div>

        <div className="grid gap-5 px-6 py-5 lg:grid-cols-[0.82fr_1.18fr]">
        ...
        <div className="flex flex-col justify-between gap-3 border-t border-slate-200 px-6 py-5 sm:flex-row sm:items-center">
