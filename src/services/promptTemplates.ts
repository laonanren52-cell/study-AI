...
## 真实初高中考试题质量要求
- 每道题必须有具体情境：至少包含具体材料、数据、条件、句子、实验、区域或历史/生活事件之一。
- 每道题必须有明确设问，不能只写“关于某概念的理解正确的是”“下列说法正确的是”。
- 每道题必须给出具体标准答案，不能写“言之有理即可”“从多个角度分析”等泛化方法论。
- 每道题必须给出详细解析，解析要能指导学生下一次怎么做。
- 每道题必须写常见误区，且误区要对应本题条件。
- 每道题必须绑定上传资料中的知识点、资料依据和 sourceEvidence。
- 不要空泛概念题，不要模板题，不要所有题都使用同一个题干结构。
- 简答题、解答题、材料分析题必须提供 scoringRubric，每个得分点要具体。
  ...
  "questions": [
    {
      ...
      "scoringRubric": ["具体得分点1", "具体得分点2"],
      "commonMistake": "对应本题条件的常见误区"
    }
  ]
}`,
  };
